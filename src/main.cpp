...
uint256 hashGenesisBlock("0x5fb62f7c1c6b78074cb25347b3db26092353fd95b27284f2965bec1310a3b3d3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WLC: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WLC Signed Message:\n";
...
{
    // WLC: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  60 * 60; // WLC: 
static const int64 nTargetSpacing = 1 * 60 ; // WLC:
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WLC: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // WLC: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // WLC: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "WLC is a software nLockTime system scriptPubKey by WLC Nakamoto in 2008.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xccb371f7b483f6665f3bf0fd1e5fa5575a0cf93c0f880ad13d9a44f7c4b42d93"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdd }; // WLC: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WLCMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WLCMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WLCMiner(CWallet *pwallet)
{
    printf("WLCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WLC-miner");
    ...
    try { loop {
        ...
        printf("Running WLCMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WLCMiner, pwallet));
}
